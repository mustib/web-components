...
import { closestPierce, EventAction, parseJson } from "@mustib/utils/browser";
...
`
   ...
   */
  getNavigationItem<T>(data: { fromIndex?: number, direction: 'next' | 'prev', switchBack?: boolean, items: T[], isNavigable: (item: T) => boolean, shouldBreak?: (item: T) => boolean }) {
    ...
    const getItem = ({ endIndex, startIndex }: { startIndex: number, endIndex: number }) => {
