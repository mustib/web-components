...
export class MuRangeThumb extends MUElement {
  ...
  _isRangeReady = this.generateIsReadyPromise({
    onTimeout: () => {
      console.warn(
        'range not set for mu-range-thumb, switching to default value [0, 100]',
        this,
      );
      this.range = [0, 100];
    },
  ...
  set range(value: [number, number]) {
    if (
      this._isRangeReady.resolved &&
      (value[0] > this.minValue || value[1] < this.maxValue)
    ) {
      console.warn(
        `range must not be less than max value (${this.maxValue}) or greater than min value (${this.minValue}), but got (${value})`,
        this,
      );
      return;
    }
  ...
  setValueFromPercentage(percentage: number): boolean {
    const value =
      (percentage / 100) * (this.range[1] - this.range[0]) + this.range[0];
  ...
  override async updated(
    ...
    if (_changedProperties.has('value')) {
      if (!this.isValidValue(this.value)) {
        console.warn(
          `value attribute must not be greater than max value (${this.maxValue}) or less than min value (${this.minValue}), but got (${this.value})`,
          this,
        );
      }
      this.style.setProperty(
        '--mu-range-thumb-percentage',
    ...
    if (
      _changedProperties.has('minValue') ||
      ...
      if (this.minValue > this.maxValue) {
        console.warn(
          `minValue attribute must not be greater than maxValue attribute, but got (${this.minValue}, ${this.maxValue})`,
          this,
        );
      } else if (
        this.minValue < this.range[0] ||
        ...
        console.warn(
          `minValue (${this.minValue}) and maxValue (${this.maxValue}) attributes must be equal to or in the range of range property (${this.range})`,
          this,
        );
      }
    }
