...
import { debounce, getElementBoundaries, throttle, wait } from "@mustib/utils/browser";
...
type Events = {
   ...
   */
  'mu-range-empty-area': CustomEvent<{ values: ReturnType<MuRange['_getValuesFromEvent']> }>,
   ...
   */
  'mu-range-added-thumbs': CustomEvent<{ data: { name: string, value: number }[] }>

  /**
   * Emitted when thumbs are removed after the initial render.
   */
  'mu-range-removed-thumbs': CustomEvent<{ data: { name: string, value: number }[] }>
}
...
export class MuRange extends MUElement {
  ...
  set activeThumb(thumb: RangeThumb | undefined) {
    if (thumb && (thumb.element.transparent || thumb.element.disabled || thumb.element.readonly)) return
  ...
  constructor() {
    ...
    this.updateComplete.then(() => {
      ...
      this.addEventListener('focus', (e) => {
        if (this._isPointerDown || this.disabled || this.readonly || e.defaultPrevented) return
        if (this._activeThumb) {
          e.preventDefault()
  ...
  dispatchChangeEvent(thumbs = this._thumbs as { name: string, value: number }[]) {
    const eventName = 'mu-range-change'
    const data = thumbs.map<Events[typeof eventName]['detail']['data'][number]>(thumb => ({ name: thumb.name, value: thumb.value }));

    this.dispatchEvent(new CustomEvent<Events[typeof eventName]['detail']>(eventName, { bubbles: true, composed: true, detail: { data: data }, cancelable: true }))
  }
  ...
  _setThumbValue(thumb: RangeThumb, value: number) {
    ...
    if (this.isControlled) {
      this.sortThumbs()
      const sortedThumbs = this._thumbs.map(thumb => ({ name: thumb.name, value: thumb === thumb ? stepValue : thumb.value }))
  ...
  _pointerdownHandler = ((e: PointerEvent) => {
    ...
    const thumbEl = MUElement.closestPierce('mu-range-thumb', e.target as HTMLElement) as MuRangeThumb | null

    if ((thumbEl && (thumbEl.disabled || thumbEl.readonly) || this.emptyArea === 'prevent')) return
  ...
  protected override async firstUpdated(_changedProperties: PropertyValues<this>): Promise<void> {
    ...
    await this.updateComplete;
    if (_changedProperties.has('defaultValue') && !this.isControlled) this.setValueFromString(this.defaultValue || '')
  }
