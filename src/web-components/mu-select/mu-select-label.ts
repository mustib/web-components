...
export class MuSelectLabel extends MUElement {
  ...
  protected _assignLabelAndValueTypes() {
    switch (this.type) {
      ...
      default:
        ...
        this._valueType = 'value';
        console.warn(
          `unsupported type (${this.type}) for mu-select-label`,
          this,
        );
        break;
    }
  ...
  protected _slotChangeHandler = () => {
    const addElement = (element: Element) => {
      if (element instanceof MuSelectLabelContent) {
        switch (element.type) {
          case 'label':
            if (!this.hasAutocomplete) this.labelElement = element;
            break;
          case 'value':
            if (this._valueType === 'value') this.valueElement = element;
            break;
          case 'autocomplete':
            if (this.hasAutocomplete) this.labelElement = element;
            break;
          case 'template':
            if (this._valueType === 'template') this.valueElement = element;
            break;
          default:
    ...
    if (!this.labelElement) {
      console.warn(
        'mu-select-label should have mu-select-label-content for label',
        this,
      );
    } else {
    ...
    if (!this.valueElement && this._valueType !== 'autocomplete-label') {
      console.warn(
        'mu-select-label should have mu-select-label-content for value',
        this,
      );
    }
  ...
  protected _addEventActionAttributes() {
    ...
    toggleOpenIcon?.setAttribute(
      'data-select-pointerdown',
      '#nothing && #prevent',
    );

    removeAllIcon?.setAttribute('data-select-click', 'remove-all');
    removeAllIcon?.setAttribute(
      'data-select-pointerdown',
      '#nothing && #prevent',
    );
  ...
  protected get _autocompleteInput() {
    if (this._labelType !== 'autocomplete') return undefined;
  ...
  get activeTemplateValue() {
    if (this._valueType !== 'template') return undefined;
