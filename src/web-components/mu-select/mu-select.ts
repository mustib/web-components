...
export class MuSelect extends MUElement {
  ...
  static eventAction = new EventAction<GenerateData<MuSelect>>({
    ...
    actions: {
      ...
      'switch-active-item'({ event, actionParam }) {
        ...
        if (dir === 'next') event.currentTarget._itemsElement?.switchActiveItem('next', { switchBack: switchBack === 'true' })
        else if (dir === 'prev') event.currentTarget._itemsElement?.switchActiveItem('prev', { switchBack: switchBack === 'true' })
      },
      ...
      'switch-active-template'({ event, actionParam }) {
        if (actionParam === 'next') event.currentTarget._labelElement?.switchActiveTemplate('next')
        else if (actionParam === 'prev') event.currentTarget._labelElement?.switchActiveTemplate('prev')
      ...
      filter(data) {
        const select = data.event.currentTarget
        if (!select.interactable) return
        const hasValue = (el: any): el is { value: string } => typeof el?.value === 'string'
    ...
    switches: {
      ...
      'has-active-item'(data) {
        return (data.event.currentTarget as MuSelect)._itemsElement?.activeValue !== undefined
      },
      ...
      template(data) {
        return (data.event.currentTarget as MuSelect)._labelElement?.hasTemplate === true
      },
      ...
      'has-active-template'(data) {
        return !!(data.event.currentTarget as MuSelect)._labelElement?.activeTemplateValue
      },
      ...
      autocomplete(data) {
        ...
        if (data.switchParam === 'both') return select._labelElement?.hasAutocompleteBoth === true
        return select._labelElement?.hasAutocomplete === true
      },

      'no-autocomplete'(data) {
        return (data.event.currentTarget as MuSelect)._labelElement?.hasAutocomplete !== true
      },

      'has-autocomplete-value'(data) {
        return !!(data.event.currentTarget as MuSelect)._labelElement?.autocompleteValue
      },
  ...
  override eventActionData: { eventAction: EventAction; events: string[]; } | undefined = {
