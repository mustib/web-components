...
import { getElementBoundaries, disableElementScroll, enableElementScroll, debounce, type GenerateData } from "@mustib/utils/browser";
...
export class MuSelectItems extends MUElement {
  ...
  static eventAction = new EventAction<GenerateData<MuSelectItems>>({
    ...
    actions: {
      'toggle-select'(data) {
        ...
        const item = items._itemsValuesMap.get(data.actionParam as any);
  ...
  protected _getNavigationItem({ direction, switchBack = false, fromIndex = direction === 'next' ? -1 : this._items.length }: { fromIndex?: number, direction: 'next' | 'prev', switchBack: boolean }) {
  ...
  protected _addActiveItemState(item: SelectItem | undefined) {
    if (!item || item.element.disabled || item.element.readonly || item.element.filteredOut) return;
  ...
  protected _dispatchActiveItemChange({ id }: { id: string | undefined }) {
    ...
    this.dispatchEvent(new CustomEvent<MuSelectItemsEvents[typeof eventName]['detail']>(eventName, { bubbles: true, composed: true, detail: { id } }))
  }
  ...
  protected getMuSelectItemFromEvent(e: Event) {
    ...
    if (item && !(item.disabled || item.readonly || item.filteredOut)) return this._itemsElementsMap.get(item);
  }
  ...
  protected _slotChangeHandler = () => {
    ...
    (this.renderRoot.querySelector('slot#items-slot') as HTMLSlotElement).assignedElements({ flatten: true }).forEach((child) => {
    ...
    if (removedSelected.size > 0) {
      ...
      this.dispatchEvent(new CustomEvent<MuSelectItemsEvents[typeof eventName]['detail']>(eventName, { bubbles: true, composed: true, detail: { orphanValues: Array.from(removedSelected), values: Array.from(newSelectedValues) } }))
    }
  ...
  _dispatchChangeEvent(detail: MuSelectItemsEvents['mu-select-items-change']['detail']) {
    const event = new CustomEvent<MuSelectItemsEvents['mu-select-items-change']['detail']>('mu-select-items-change', {
      detail,
      bubbles: true,
      composed: true,
      cancelable: true
  ...
  _debounceDispatchChangeEvent = debounce(this._dispatchChangeEvent.bind(this), 50)
   ...
   */
  protected _changeSelectState(type: 'add' | 'add-only' | 'remove' | 'toggle', _item: SelectItem | string | undefined) {
   ...
   */
  protected _changeSelectStateFromAttributeValue(value: string | undefined | null | string[]) {
    if (!value) return
    const validValues = (Array.isArray(value) ? value : value.split(',')).filter(value => {
      if (value === '') return false
      const isValid = this._itemsValuesMap.has(value);
      if (!isValid) console.warn(`No item found for value (${value}) in select items`, this);
      return isValid
    })
    ...
    this.dispatchEvent(new CustomEvent<MuSelectItemsEvents[typeof eventName]['detail']>(eventName, { bubbles: true, composed: true, detail: { values: validValues } }))
  }
  ...
  protected override async firstUpdated(_changedProperties: PropertyValues<this>): Promise<void> {
    await this._isReady.promise
