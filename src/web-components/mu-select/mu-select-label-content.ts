  ...
  async setValue(value: string[]) {
    ...
    switch (this.type) {
      ...
      case "value":
        ...
        break;
      case "autocomplete":
        if (contentEl && contentEl instanceof HTMLInputElement) contentEl.value = value.join(', ')
        else console.warn(`mu-select-label-content with type="autocomplete" should have an input element with ${contentSelector} attribute to change the value`, this)
        break;
      case "template":
        if (this.activeTemplateValue && !value.includes(this.activeTemplateValue)) this.activeTemplateValue = undefined
        this.requestUpdate()
        break;
      default:
